...
${USE_CHECKER:+compute-sanitizer --tool memcheck --target-processes=all --report-api-errors=all} o2-its-reco-workflow ${GLOSET} --trackerCA --tracking-mode async --disable-mc --digits-from-upstream --use-gpu-workflow --gpu-device=${GPU_DEVICE} ${ITS_RECOPAR} -b --run  2>&1 | sed 's/\[[^]]*:[^]]*\]//g' | tee cuda.log
