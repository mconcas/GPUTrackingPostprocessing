...
o2-ctf-reader-workflow ${GLOSET} --severity error --max-tf=${MAXTF:-1} --ctf-input data.lst --onlyDet ITS --allow-missing-detectors --its-digits --configKeyValues ";;" |
    o2-its-reco-workflow ${GLOSET} --trackerCA ${ITS_TRACKING_MODE} --disable-mc --digits-from-upstream --use-gpu-workflow --gpu-device=${GPU_DEVICE} ${ITS_RECOPAR} -b --run 2>&1 | sed 's/\[[^]]*:[^]]*\]//g' | tee hip.log
